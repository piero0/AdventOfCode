...
getShrtLen :: (Pt, Int) -> (Pt, Int) -> Int
getShrtLen (s, ds) (d, dd) = if dxy <= 20 then dd - ds - dxy else -1
  where
    dxy = abs (fst s - fst d) + abs (snd s - snd d)

part2 :: [(Pt, Int)] -> [Int]
part2 path = filter (>= 100) [getShrtLen x y | x <- path, y <- path]
